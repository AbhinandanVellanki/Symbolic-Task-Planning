...
def CheckCondition(state, condition):
    ...
    """
    if (
        np.sum(np.multiply(state, condition))
        - np.sum(np.multiply(condition, condition))
    ) == 0:
        return True
    else:
        return False
...
def InitializePreconditionsAndEffects(nrObjects, nrPredicates, Predicates, Objects):
    ...
    for i in range(1, 5, 1):
        ...
        Effect[0][0] = 2.0  # Robot in the hallway
        Effect[0][i] = -2.0  # Robot not in the i-th room
    ...
    for i in range(1, 5, 1):
        ...
        Effect = np.zeros([nrObjects, nrPredicates])
        Effect[0][0] = -2.0  # Robot not in the hallway
        Effect[0][i] = 2.0  # Robot in the ith room
    ...
    ### Move to Pantry
    # Robot in the kitchen and Robot not in the pantry
    Precond = np.zeros([nrObjects, nrPredicates])
    Precond[0][5] = -1  # Robot not in the pantry
    Precond[0][1] = 1  # Robot in the kitchen

    # Move robot from the kitchen to the pantry (remove from kitchen and add to pantry)
    Effect = np.zeros([nrObjects, nrPredicates])
    Effect[0][1] = -2.0  # Move robot out of the kitchen
    Effect[0][5] = 2.0  # Move robot into the pantry
    ...
    ### Move from Pantry
    # Robot not in the kitchen and Robot in the pantry
    Precond = np.zeros([nrObjects, nrPredicates])
    Precond[0][1] = -1  # Robot not in the kitchen
    Precond[0][5] = 1  # Robot in the pantry

    # Move robot from the pantry to the kitchen (remove from pantry and add to kitchen)
    Effect = np.zeros([nrObjects, nrPredicates])
    Effect[0][5] = -2.0  # Move robot out of the pantry room
    Effect[0][1] = 2.0  # Move robot into the kitchen
    ...
    for j in [1, 2]:
        # Robot in the kitchen, fruit in the kitchen, knife in the kitchen, fruit not chopped
        Precond = np.zeros([nrObjects, nrPredicates])
        Precond[0][1] = 1  # Robot in the kitchen
        ...
        # Fruit is chopped
        Effect = np.zeros([nrObjects, nrPredicates])
        Effect[j][6] = 2.0  # Fruit is chopped
    ...
    for i in range(1, 6, 1):
        for j in range(1, 5, 1):
            ...
            Effect = np.zeros([nrObjects, nrPredicates])
            Effect[j][i] = 2.0  # Object j in ith room
...
ActionPre, ActionEff, ActionDesc = InitializePreconditionsAndEffects(
    nrObjects, nrPredicates, Predicates, Objects
)
...
while len(pq) > 0:
    ...
    for i in range(len(ActionPre)):
        ...
        if not CheckCondition(vertices[x], ActionPre[i]):
            continue

        # Compute the next state
        nextstate = ComputeNextState(vertices[x], ActionEff[i])

        # Check if the next state is already visited
        if not CheckVisited(nextstate, vertices):
            # Compute the cost
            new_cost = cost2come[x] + 1  # Assuming each action has a cost of 1
            #heapq.heappush(pq, (new_cost + Heuristic(nextstate, GoalIndicesOneStep, GoalIndicesTwoStep), len(vertices))) # A*
            heapq.heappush(pq, (new_cost, len(vertices))) # Dijkstra

            # Append the next state to vertices, parent, action, and cost2come
